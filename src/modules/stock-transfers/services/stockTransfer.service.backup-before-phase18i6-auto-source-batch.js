...
const validateAndBuildRequestItems = async (items, fromBranchId) => {
  const builtItems = [];
...
const createStockTransferRequest = async (payload, actor) => {
  ...
  const fromBranch = await getActiveBranchOrThrow(payload.fromBranchId, "From branch");
  ...
  if (!toBranchId) {
    throw new AppError("To branch is required", 400, "TO_BRANCH_REQUIRED");
  }

  const toBranch = await getActiveBranchOrThrow(toBranchId, "To branch");
  ...
  const items = await validateAndBuildRequestItems(payload.items, fromBranch.id);
